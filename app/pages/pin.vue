...
<template>
  <main class="flex min-h-screen items-center justify-center px-6 py-12">
    <form class="w-full max-w-sm" @submit.prevent="submit">
      ...
      <p class="mb-3 text-xs font-semibold uppercase tracking-[0.2em] text-accent">{{ t('app.name') }}</p>
      <h1 class="text-4xl font-semibold tracking-tight">{{ t('pin.title') }}</h1>
      <p class="mt-4 text-base leading-7 text-stone-600">{{ t('pin.description') }}</p>
      <label for="pin" class="mt-10 block text-sm font-medium">{{ t('pin.label') }}</label>
      ...
      <Button class="mt-5 h-13 w-full rounded-xl text-base" :disabled="pin.length !== 4 || submitting">{{ submitting ? t('pin.checking') : t('pin.submit') }}<ArrowRight v-if="!submitting" aria-hidden="true" /></Button>
    </form>
  </main>
</template>
